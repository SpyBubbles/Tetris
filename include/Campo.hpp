...
class Campo : public sf::Drawable { // La clase Campo hereda de sf::Drawable para poder ser dibujada en una ventana de SFML
private:
...
void Campo::ActColoresTablero(){ // Actualiza los colores de las celdas del tablero
    for(int i = 0; i < 20; i++){
        for(int j = 0; j < 10; j++){
            switch (tablero[i][j]){
                case 0:                  
